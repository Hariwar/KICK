...
from bot import VID_MODE, config_dict
...
class SelectMode:
    ...
    def _captions(self, mode: str | None = None):
        msg = (
            f"<b>VIDEOS TOOL SETTINGS</b>\nMode: <b>{vidmode}</b>"
        ...
        if self.mode in ("vid_sub", "watermark"):
            ...
            if hardsub:
                ...
                if (
                    fontname := self.extra_data.get("fontname")
                    or config_dict["HARDSUB_FONT_NAME"]
                ):
                    msg += f"\nFont Name: <b>{fontname.replace('_', ' ')}</b>"
                if (
                    fontsize := self.extra_data.get("fontsize")
                    or config_dict["HARDSUB_FONT_SIZE"]
                ):
    ...
    async def list_buttons(self, mode: str = ""):
        ...
        if not mode:
            vid_modes = (
                dict(list(VID_MODE.items())[4:]) if self._isLink else VID_MODE
            )
            ...
            match mode:
                ...
                case "fontstyle":
                    ...
                    buttons.button_data(
                        f"{'✅ ' if self.extra_data.get('boldstyle') else ''}Bold Style",
        ...
        case "quality" | "popupwm" as value:
            if len(data) == 3:
                obj.extra_data[value] = (
                    data[2] if value == "quality" else int(data[2])
                )
