...
class ExtraSelect:
    ...
    def streams_select(self, streams: dict | None = None):
        ...
        if not self.executor.data:
            ...
            for stream in streams:
                indexmap, codec_name, codec_type, lang = (
                    stream.get("index"),
    ...
    async def convert_select(self, streams: dict):
        ...
        buttons.button_data("Cancel", "extra cancel", "footer")
        await self.update_message(
            f"{self._listener.tag}, Select available resulution to convert.\n<code>{self.executor.name}</code>",
            buttons.build_menu(2),
        )
        ...
        if not self.status:
            for possition, file in self.executor.data["list"].items():
                if file.endswith(("srt", ".ass")):
                    ref_file = (
                        self.executor.data["final"].get(possition, {}).get("ref", "")
                    )
            ...
            for possition, file in self.executor.data["list"].items():
                if (
                    possition != self.status
                    and file not in self.executor.data["final"].values()
                ):
                    text += f"{index}. {file}\n"
    ...
    match data[1]:
        ...
        case "subsync":
            if data[2].isdigit():
                ...
                obj.executor.data["final"][obj.status]["ref"] = obj.executor.data[
                    "list"
                ][int(data[3])]
                obj.status = ""
